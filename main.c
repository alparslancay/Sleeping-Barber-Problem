...
void initializeSemaphores(sem_t *lock, sem_t *forks, int num_forks);
void runAllThreads(pthread_t *threads, sem_t *forks, sem_t *lock, int num_philosophers);
...
int main(int argc, char *args[])
{
    ...
    initializeSemaphores(&lock, forks, num_philosophers);
    runAllThreads(philosophers,forks,&lock,num_philosophers);
    pthread_exit();
    return 0;
}
...
int i;
    for(i=0;i<num_forks;i++)
        sem_init(&forks[i],0,1);//TODO
    sem_init(lock,0,num_forks -1);//TODO
}
